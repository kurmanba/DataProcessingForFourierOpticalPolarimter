...
def map_performance(ratio: float,
    ...
    determination = 83
    theta_1 = np.linspace(100, 170, angular_increments)
    theta_2 = np.linspace(100, 170, angular_increments)
...
#
# # # ratios = np.array([3.68])         # np.linspace(1, 6, 6)
# ratios = np.array([7.92055192])       # np.linspace(1, 6, 6)
#
# for i in tqdm(ratios):
#     map_performance(i, 3.38026534, 120)


def drr_norm_measure(v: np.ndarray) -> float:
