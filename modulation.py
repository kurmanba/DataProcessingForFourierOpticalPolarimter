...
def modulation_matrix4(theta1: any,
                       theta2: any,
                       retardance1: any,
                       retardance2: any,
                       q: any) -> np.ndarray:
...
theta_1 = np.linspace(0, 360, 100)
theta_2 = np.linspace(0, 360, 100)
...
for i, x in (enumerate(theta_1)):
    for j, y in enumerate(theta_2):
        h = modulation_matrix4(x, y, 173, 71, 1)
        for q in range(2, 17):
            h2 = modulation_matrix4(x, y, 173, 71, q)
            h = np.vstack((h, h2))
